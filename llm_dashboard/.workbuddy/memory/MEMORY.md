...
- **v20260821改动(UI)**: 新增板块七「各类目星期表现对比」(类目×星期热力表 + 6指标分段控件 + 预算分配建议)；有效状态筛选从顶部筛选栏迁移至板块六卡片旁(标注"全局")；板块五匹配方式增加CVR；原板块七优化建议重编号为板块八。板块七色块对比增强(中值差异1.35倍放大+透明度0.16~0.72)；星期列序改为 周一~周六→周日置末(WK_ORDER)；目标看板模块二【负责人】【国家】新增费比·昨日/上周/4周 三列(实际TACOS环比pp, ▲红▼绿)，国家卡片下移至负责人下方。版本号 v20260821d2（本次已 bump，无需强制刷新）
